...
It is thus fully compatible with packages such as \pkg{lubridate} [@grolemundDatesTimesMade2011] and \pkg{anytime} [@eddelbuettelAnytimeEasierDate2019].
...
`is_intersecting()` tests whether there is any intersection between two messy dates. `is_element()` similarly tests whether a messy date can be found within
...
Take the dates of battles in 2001 according to [Wikipedia](https://en.wikipedia.org/wiki/List_of_battles_in_the_21st_century) included in \pkg{messydates}.
The dates of these battles are often approximate
(i.e. the day in which a battle started or ended is not precise)
or come from unreliable sources (i.e. the date is precise but not trustworthy).
