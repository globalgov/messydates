...
# Working with messydates: The size of the Roman Empire Population
...
We can, for instance, coerce the date variable in the Roman population data.
...
For example, we can add these annotations to the date variable in the Roman population data.
...
The ranges in the date variable in the Roman population data can be also expanded.

```{r expand, warning=FALSE}
expand(roman_population$period)
```
...
We can contract back the date variable in the Roman population previously expanded.

```{r contract, warning=FALSE}
contract(expand(roman_population$period), collapse = FALSE)
```
...
We can coerce the date variable in the Roman population to `Date` class.
...
For example, we can proceed to check various logical tests for the date variable in the Roman population.
...
md_union(as_messydate("-0201-12-31"), as_messydate(roman_population$period[1]))
md_multiset(as_messydate("-0200-01-01"), as_messydate(roman_population$period[1]))
```
