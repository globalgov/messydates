test_that("Logical comparisons work", {
  ...
  expect_true(as.POSIXct("2012-06-01", tz = "UTC") <= as_messydate("2012-06-XX"))
  expect_equal(as.Date("2012-06-01") >= as_messydate("2012-06-XX"), NA)
  expect_equal(as.POSIXct("2012-06-01", tz = "UTC") >= as_messydate("2012-06-XX"), NA)
  expect_equal(as_messydate("2012-06-XX") >= as.Date("2012-06-02"), NA)
  expect_equal(as_messydate("2012-06-XX") >= as.POSIXct("2012-06-02", tz = "UTC"), NA)
...
p1 <- as.POSIXct(c("2012-01-01", "2012-02-01", "2012-03-01"), tz = "UTC")
p2 <- as.POSIXct(c("2012-03-01", "2012-02-01", "2012-01-01"), tz = "UTC")
...
test_that("Logical comparisons account for time of day on the same date", {
  # Regression test: comparisons used to compare via as.Date(), silently
  # dropping the time of day and treating same-day date-times as equal.
  early <- as_messydate("2012-06-02T09:00:00")
  late <- as_messydate("2012-06-02T17:00:00")
  expect_true(early < late)
  expect_false(early > late)
  expect_true(early <= late)
  expect_false(early >= late)
  expect_true(late > as.POSIXct("2012-06-02 09:00:00", tz = "UTC"))
})
