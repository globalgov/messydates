...
Getting the timing can be important for researchers.
However, when faced with date imprecision, researchers usually have to choose
between making arbitrary choices (e.g. adding "-01-01" to all incomplete dates)
or work with imprecise dates (e.g. keep the year only).
Yet, either choice may lead to biased results.
...
A relationship that could be mediated by the number of parties involved in the conflict.
...
one variable with an arbitrary cut off point and the other variable
...
In this case, setting arbitrary cut off points to dates introduces a highly
