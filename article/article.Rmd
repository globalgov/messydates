...
<!-- Should we have a separate motivation section? If so, I would suggest we merge the first paragraph of the introduction and the quick overview section, to become the introduction and expand on the second paragraph of the introduction to become the motivation section. -->
...
<!-- Should we mention/illustrate the ability to do some arithmetic operations with messydates here? -->
...
emperors <- manydata::consolidate(database = manydata::favour(manydata::emperors, "UNRV"),
  ...
  mutate(years_only = as.numeric((as.Date(End, max) - as.Date(Beg, min))/365.25),
         arbitrary = as.numeric(as.Date(End, max) - as.Date(Beg, min)),
         random = as.Date(End, random) - as.Date(Beg, random))
