...
For example, we can proceed to run logical tests to the 'mdate' variable in the Roman population.
...
Additionally, one can perform intersection (`md_intersect()`), union (`md_union()`),
or 'join' the 'mdate' variable in the Roman population.
...
```{r operate}
roman_population$period + 1 # Adds one day from ranges
