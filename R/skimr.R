...
#'
#' @description
#' Defines a `{skimr}` template for messydt objects. The helper functions take a
#' `{messydt}` vector as an input and return a single value to be displayed
#' by the `{skimr}` report. The vector is first resolved yielding a single
