...
Several historical facts are contested. Take, for example, the population sizes in the Roman Empire, which has been a topic of heated debates among some academics. Various accounts diverge in how they estimate population size and, therefore, the conclusions about the Roman population at different points in time [@scheidel2008]. Below, we exemplify how \pkg{messydates} facilitate the annotation and the conversion of historical dates for analysis by building a dataset of the high and low estimations for the population of mainland Italy during the Roman Empire [@scheidel2008, p. 31].
...
Assume, for example, certain researcher is interested in the relationship between how Roman Emperors rose to power and how long they stayed in power. The researcher theorizes that Roman Emperors that rose to power through birthrights were less contested and, thus, tended to stay in power for longer than those Roman Emperors that rose to power in other ways (e.g. coup or appointment). 

The \pkg{manydata} contains a database on Roman Emperors composed of three datasets. We can also use \pkg{manydata} to "consolidate" the database into a single dataset using some combination of the rows and columns available. Below, we use the `consolidate()` function to transform the Emperors database into a single dataset by favoring the United Nations of Rome Victrix (UNVR) dataset. Then, we use \pkg{messydates} to create three different date variables to be our dependent variables related that represents time in power: imprecise (year only), arbitrary (the minimal value for a data range), and random (a random value within the date range). With these variables in hand, we build three simple linear regression models correlating each of the dependent variables to the variable on how Roman Emperors rose to power.
...
emperors <- manydata::consolidate(database = manydata::favour(manydata::emperors, "UNRV"),
                                  rows = "every", # rows in every dataset
                                  cols = "any", # cols in any dataset
                                  resolve = "coalesce", # first non-NA value
                                  key = "ID") %>% # commem variable
